...
// Groq's JSON-mode validator occasionally rejects the model's own generation
// as invalid JSON (HTTP 400, error.code "json_validate_failed") — this is
// non-deterministic model sampling variance, not a bug in our request, and a
// retry of the identical request typically succeeds. Capped at one retry so
// a persistently malformed prompt still fails fast.
...
export async function generateDoubtHint(input) {
  ...
  const parsed = await callGroqJSON({
    ...
    maxTokens: input.hintLevel === 4 ? 900 : 450,
    temperature: 0.3,
  });
...
function buildFullSolutionUserPrompt({
  ...
  const parts = [
    ...
    currentCode
      ...
      : "Student's Current Code: (none provided)",
    doubt ? `Student's Original Doubt: ${doubt}` : null,
    previousHints?.length
