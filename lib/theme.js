...
export const DARK_STATUS_STYLES = {
  ...
  NEUTRAL: "bg-white/5 text-slate-400 border-slate-800",
};
...
export const DARK_PRIORITY_STYLES = {
  ...
  Low: "bg-white/5 text-slate-400 border-slate-800",
};

// AlgoLens global dark palette — deep blue-black + indigo + violet, not pure
// black/gray. Surface tones (background/nav/card) don't map to a single
// stock Tailwind shade family, so they're kept as explicit hex values here
// as the one source of truth; text/borders/accents below use the named
// Tailwind classes that happen to compute to the same hex (slate-50/400,
// violet-500, indigo-500, blue-500) since those read more clearly in
// component code.
export const DARK_BG = "bg-[#0B0F17]"; // main background — rich deep obsidian, not pure black
export const DARK_NAV_BG = "bg-[#0D1220]"; // navbar/sidebar
export const DARK_CARD_BG = "bg-[#151C28]"; // cards/elevated surface panels

export const DARK_CARD = `${DARK_CARD_BG} border border-white/10 rounded-xl`;
export const DARK_CARD_PADDED = `${DARK_CARD} p-5 sm:p-6`;
// Opt-in hover treatment for clickable cards/rows: a subtle violet glow
// (not every DARK_CARD is interactive, so this isn't baked into DARK_CARD
// itself).
export const DARK_CARD_HOVER = "hover:border-violet-500/30 hover:shadow-md hover:shadow-violet-500/5";

// Category/topic pills — semi-transparent slate, distinct from status/
// priority badges (which stay in their semantic red/amber/green/blue).
export const DARK_TOPIC_PILL = "bg-slate-800/80 text-slate-300 border border-slate-700/50";

export const DARK_BTN_PRIMARY =
  "inline-flex items-center justify-center gap-1.5 px-5 py-2.5 rounded-lg bg-gradient-to-r from-violet-500 to-indigo-500 text-white text-sm font-semibold transition-all hover:-translate-y-0.5 hover:shadow-lg hover:shadow-indigo-950/40 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0";
export const DARK_BTN_SECONDARY =
  "inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-lg border border-slate-800 text-slate-200 text-sm font-medium transition-colors hover:bg-[#151D2E] hover:border-slate-700 disabled:opacity-50 disabled:cursor-not-allowed";
