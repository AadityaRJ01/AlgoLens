  ...
  if (mistakes.length === 0) {
    return (
      <div>
        ...
        <p className="mt-1.5 text-sm text-slate-500">No failure analyses for this concept yet.</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-3">
        <p className="text-xs font-semibold uppercase tracking-wide text-rose-400/90">Common Mistakes</p>
        <span className="text-[11px] text-slate-600">Detected across {submissionCount} recent submission{submissionCount === 1 ? "" : "s"}</span>
      </div>
      <ul className="mt-1.5 space-y-1.5">
        {mistakes.map((m) => (
          <li key={m.id} className="flex items-start gap-2 text-sm text-slate-300">
