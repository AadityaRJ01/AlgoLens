    ...
    <div className="space-y-4">
      <div className={DARK_CARD_PADDED}>
        ...
        <div className="mt-2 flex items-center gap-2 flex-wrap">
          ...
          <span className={`text-sm font-semibold ${DARK_DIFFICULTY_STYLES[problem.difficulty] || "text-slate-400"}`}>
            {problem.difficulty}
          </span>
        </div>
        ...
        <div className="mt-3 border-t border-slate-800 pt-3 text-xs">
          ...
          <span className="mx-1.5 text-slate-700">·</span>
          <span className="text-slate-500">{new Date(submission.timestamp).toLocaleDateString()}</span>
        </div>
              ...
              <div className="mt-1 space-y-1">
                {examples.map((example, i) => (
                  <details key={i} className="rounded-md border border-white/10 bg-white/[0.02] px-2.5 py-1.5" open={i === 0}>
                    <summary className="cursor-pointer text-xs font-medium text-slate-300">▸ Example {i + 1}</summary>
                    <p className="mt-1.5 whitespace-pre-wrap text-xs leading-relaxed text-slate-400">{example}</p>
                  </details>
                ))}
              </div>
            </div>
          ...
          {constraints && (
            <div>
              ...
              <details className="mt-1 rounded-md border border-white/10 bg-white/[0.02] px-2.5 py-1.5">
                <summary className="cursor-pointer text-xs font-medium text-slate-300">▸ Show constraints</summary>
                <p className="mt-1.5 whitespace-pre-wrap text-xs leading-relaxed text-slate-400">{constraints}</p>
              </details>
            </div>
          )}
        ...
        {problem.url && (
          <a
            ...
            className="mt-3 inline-block border-t border-slate-800 pt-3 text-xs font-medium text-indigo-400 hover:text-indigo-300"
          >
            View on LeetCode ↗
          </a>
        )}
      ...
      {recentSubmissions.length > 1 && (
        <div className={DARK_CARD_PADDED}>
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Recent Submissions</p>
              ...
              <li key={s.id}>
                <Link
                  ...
                  className={`flex items-center justify-between gap-2 rounded-lg px-2.5 py-1.5 text-sm transition-colors ${
                    s.isSelected ? "bg-indigo-500/10 text-slate-50 ring-1 ring-inset ring-indigo-400/25" : "text-slate-400 hover:bg-white/5"
                  }`}
                >
